...
class MultiHeadedAttentionGumbel(nn.Module):
    """Multi head attention module with different input dimension.
    Use Gumbel softmax to replace orignal softmax
    ...
    def __init__(self, 
        ...
        super(MultiHeadedAttentionGumbel, self).__init__()
        ...
        self.gumbel_start_step = gumbel_start_step
    ...
    def set_step(
        ...
        """Set up global_step and calculate tau"""
        self.global_step = global_step
        tau = self.gumbel_tau_func(global_step)
        if tau != self.tau:
    ...
    def gumbel_softmax_func(self, scores, dim=-1):
        ...
        elif self.gumbel_activation == 'sigmoid':
            if self.global_step >= self.gumbel_start_step:
                return gumbel_sigmoid(scores, self.tau, self.gumbel_hard)
            else:
                return torch.sigmoid(scores)
        else:
            raise ValueError(self.gumbel_activation)
    ...
    def forward_attention(self, value, scores, mask, weights=None):
        ...
        if mask is not None:
            ...
            self.attn = self.gumbel_softmax_func(scores, dim=-1).masked_fill(
                mask, 0.0
            )  # (batch, head, time1, time2)
        else:
            self.attn = self.gumbel_softmax_func(scores, dim=-1)  # (batch, head, time1, time2)
    ...
    def forward(self, query, key, value, mask, weights=None):
        ...
        output, attn = self.forward_attention(v, scores, mask, weights=weights)
