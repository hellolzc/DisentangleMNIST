...
def parse_and_update(config, kwargs, prefix=''):
    ...
    for k, v in kwargs.items():
        if isinstance(v, Dict):
            parse_and_update(config[k], v, '{}{}.'.format(prefix, k))
        else:
            if k not in config:
...
def create_hparams(fpath, yaml_hparams_string='', json_hparams_string='', debug_print=True):
    ...
    if (yaml_hparams_string is not None) and (yaml_hparams_string != ''):
        ...
        if update_dict is not None:
            parse_and_update(config, update_dict)
